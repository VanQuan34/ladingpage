import { Component, Input, Output, EventEmitter, OnChanges, SimpleChanges, ElementRef, ViewEncapsulation, ChangeDetectorRef, Renderer2, ViewChild, forwardRef, OnInit, ViewRef } from '@angular/core';
import { ControlValueAccessor, NG_VALUE_ACCESSOR } from '@angular/forms';
import { LangChangeEvent, TranslateService } from '@ngx-translate/core';
import {
  IMyDate, IMyDateRange, IMyMonth, IMyCalendarDay, IMyCalendarMonth, IMyCalendarYear,
  IMyWeek, IMyDayLabels, IMyMonthLabels, IMyOptions, IMyDateModel, IMyInputFieldChanged, IMyCalendarViewChanged, IMyInputFocusBlur, IMyMarkedDates, IMyMarkedDate, IMyDateFormat
...
export class MyDatePicker implements OnChanges, ControlValueAccessor, OnInit {
  @Input() options: any;
  @Input() locale: string;
  ...
  listYear: Array<number> = new Array<number>();
  ...
  constructor(
    ...
    this._translate.onLangChange.subscribe((evt: LangChangeEvent) => {
  ...
  selectNewYear(year: any) {
    this.visibleMonth.year = year;
  ...
  writeValue(value: any): void {
    if (value && (value['date'] || value['jsdate'] || value['formatted'])) {
      ...
      if (cvc) {
        this.visibleMonth = { monthTxt: this.monthText(this.selectedDate.month), monthNbr: this.selectedDate.month, year: this.selectedDate.year };
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (this.visibleMonth.year === 0 && this.visibleMonth.monthNbr === 0 || dmChange) {
      ...
      if (this.monthsList && this.monthsList.length) {
        this.visibleMonth.monthTxt = this.monthsList.find((item, idx) => idx + 1 === this.visibleMonth.monthNbr);
      }
  ...
  onPrevMonth(): void {
    ...
    if (this.options.disableUntil && (this.options.disableUntil.year > d.getFullYear() ||
      this.options.disableUntil.year === d.getFullYear() && this.options.disableUntil.month > d.getMonth())) {
      return;
    }
    if (d.getMonth() - 1 < 0) {
      d.setMonth(11);
    ...
    this.visibleMonth = { monthTxt: this.monthText(m), monthNbr: m, year: y };
    this.generateCalendar(m, y, true);
    this.onYearChanged.emit(y);
  }

  onNextMonth(): void {
    // Next month from calendar
    let d: Date = this.getDate(this.visibleMonth.year, this.visibleMonth.monthNbr, 1);
    if (this.options.disableSince && (this.options.disableSince.year < d.getFullYear() ||
      this.options.disableSince.year === d.getFullYear() && this.options.disableSince.month <= d.getMonth() + 1)) {
      return;
    }
    if (d.getMonth() + 1 >= 12) {
      d.setMonth(0);
  ...
  onTodayClicked(): void {
    ...
    if (this.opts.inline && today.year !== this.visibleMonth.year || today.month !== this.visibleMonth.monthNbr) {
      this.visibleMonth = { monthTxt: this.monthText(today.month), monthNbr: today.month, year: today.year };
  ...
  monthText(m: number): string {
    ...
    if (this.monthsList && this.monthsList.length) {
      return this.monthsList.find((item, idx) => idx + 1 === m);
    }
    return '';
  }
