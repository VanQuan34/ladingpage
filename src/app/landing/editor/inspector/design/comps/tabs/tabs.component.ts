...
@Component({
  selector: 'mo-wb-landing-editor-inspector-design-comps-tabs',
  templateUrl: './tabs.component.html',
  styleUrls: ['./tabs.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class MoLandingEditorInspectorLayoutCompsTabsComponent extends MoLandingEditorInspectorBaseComponent {

  @Input() selectorAddTextHover: string = ' .mo-tabs-container [name="tab"]:checked + .label-title';
  fontColor: any;
  fontColorHover: any;
  bgTitle: string;
  bgTitleHover: string;
  ...
  handleOnUpdateHeightTabContent = () => {
    ...
    let maxHeight = 0;
    const liElements = this.selectedComp.find(this.selectorTitle);
    liElements.forEach(li => {
      const liHeight = li.view.el.offsetHeight;
    ...
    this.setStyle(`${(maxHeight + gapTop + gapBottom)}px`, HEIGHT_TITLE, this.selectorContent);
  }
